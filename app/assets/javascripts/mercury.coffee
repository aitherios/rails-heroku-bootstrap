...
#= require mercury/locales/pt.locale
#
...
window.Mercury =
  ...
  config:
    ...
    injectedStyles: "
      [data-mercury]       { min-height: 10px; cursor: help !important; }
...
($ window).bind 'mercury:ready', ->
  iframeRef = document.getElementById('mercury_iframe')
  innerDocument = if iframeRef.contentWindow then iframeRef.contentWindow.document else iframeRef.contentDocument
  # Disable every link inside
  ($ 'a', innerDocument).click (event)->
    event.preventDefault()
  # Disable forms from submitting
  ($ 'form', innerDocument).submit (event)->
    event.preventDefault()
  # Prevent label tags from selecting inputs
  ($ 'label', innerDocument).attr('for', '')
